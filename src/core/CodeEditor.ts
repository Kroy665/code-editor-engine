...
export class CodeEditorEngine extends TypedEventEmitter<EditorEvents> implements CodeEditor {
  ...
  private setupBuiltInCommands(): void {
    ...
    this.disposables.add(
      this.commandRegistry.registerHandler('editor.insertText', (text: string, position?: Position) => 
    ...
    this.disposables.add(
      this.commandRegistry.registerHandler('editor.replaceText', (range: Range, text: string) => 
  ...
  private setupEventForwarding(): void {
    ...
    this.disposables.add(
      this.commandHistory.on('command-undone', ({ command }: { command: UndoableCommand }) => {
        this.emit('undo', { command });
      })
    );

    this.disposables.add(
      this.commandHistory.on('command-redone', ({ command }: { command: UndoableCommand }) => {
